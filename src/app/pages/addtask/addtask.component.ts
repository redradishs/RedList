...
export class AddtaskComponent {
  ...
  retrieveDone() {
    const currentUserId = this.userId;
    this.taskService.getStatus('done').subscribe((resp: any) => {
  ...
  retrieveInProgress() {
    const currentUserId = this.userId;
    this.taskService.getStatus('inprogress').subscribe((resp: any) => {
      let tasks = resp.data.filter((task: Task) => task.user_id === currentUserId);
      tasks = this.sortTasksByOrder(tasks);
  ...
  retrieveTodo() {
    const currentUserId = this.userId;
    this.taskService.getStatus('todo').subscribe((resp: any) => {
      let tasks = resp.data.filter((task: Task) => task.user_id === currentUserId);
      tasks = this.sortTasksByOrder(tasks);
  ...
  formatDueDate(dueDate: string, status: string): { formattedDate: string, color: string } {
    ...
    if (status !== 'done' && isPast(date) && !isToday(date)) {
      ...
      // formattedDate = `Overdue ${daysOverdue} day`;
